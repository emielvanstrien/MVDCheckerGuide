...
public class IfcChecker {
	...
	public IfcChecker(String ifcSchema,String ifcFile, List<MVDConstraint> constraints)
	...
	public void checkIfcModel(String outputFolder) throws JAXBException, DeserializeException, ParserConfigurationException, SAXException {
		ReportWriter reportWriter = new ReportWriter(ifcModel,
				ifcFile);
		reportWriter.setUpGC();
		...
		try {
			...
			for (Object ifcObject : allRoots) {
				...
				if (comment.length() > 0) {
					...
					if (ifcObject instanceof IfcWall||ifcObject instanceof IfcDoor) {
						...
						String spatialStructureElement = null;
						
						try {
							reportWriter.addIssueGC(spatialStructureElement,
